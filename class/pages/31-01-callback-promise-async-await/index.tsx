...
export default function CallbackPromiseAsyncAwaitpage() {
  ...
  function onClickMyCallback() {
    ...
    ccc.addEventListener("load", (res) => {
      ...
      aaa.addEventListener("load", (res) => {
        ...
        aaa2.open("get", `http://koreanjson.com/posts?${user}`);
        aaa2.send();
  ...
  function onClickMyPromise() {
    axios
      ...
      .then((res2) => {
        ...
        return axios.get(`http://koreanjson.com/posts?${user}`);
      })
      .then((res3) => {
  ...
  async function onClickMyAsyncAwait() {
    ...
    const res3 = await axios.get(`http://koreanjson.com/posts?${user}`);
    setMyPromise(res3.data);
  }
  ...
  return (
    <>
      ...
      <button onClick={onClickMyCallback}>콜백요청하기!</button>
      <span>
        결과:
        {myPromise.map((el) => (
          <div key={el.title}>{el.title}</div>
